...
export default function Directors() {
    ...
    const fetchActors = async (pageNo) => {
        const { profiles, error } = await getDirectors(pageNo, limit);
    ...
    const handleOnSearchSubmit = (value) => {
        handleSearch(searchDirector, value, setResults);
    };
    ...
    const handleOnDeleteConfirm = async () => {
        setBusy(true);
        const { error, message } = await deleteDirector(selectedProfile.id);
        setBusy(false);
        if (error) return updateNotification("error", error);
